...
class RolloverStrategy(bt.Strategy):
    params = (('shift_pos_days', 30),('rollover_window_prd',3), ('asset_ratio', 1),
    ...
    '''shift_pos_days : the holding days of portfolio, if overpass it, we will shift the position
       rollover_window_prd: the waiting window days uesd to calculate the skewness indicator
    ...
    def prenext(self):
        print('I am not ready, need %s days'%self.params.rollover_window_prd)
    ...
    def next(self):
        ...
        if self.holding_days == self.params.shift_pos_days:
            ...
            temp_ind = pd.Series([self.rollover_ind[vt][self.clock] for vt in self.index_campping_tradevt.values()], \
...
if __name__ == '__main__':  
    ...
    cerebro = bt.Cerebro()  
    # Add a strategy  
    cerebro.addstrategy(RolloverStrategy)  
    ...
    if False:
        ...
        indicator_byvt = stat.rollover_ind
